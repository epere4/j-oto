...
public class UiUtil {
    ...
    private static String _askTextFileContent(Shell shell, String defaultFileName) {
        ...
        String oldPath = Activator.getDefault().getPreferenceStore().getString("old.path");
        if (oldPath != null) {
        ...
        //Store pref 
        Activator.getDefault().getPreferenceStore().putValue("old.path", dialog.getFilterPath());
	...
	public static String askText(Shell shell) {
		...
		String checkRes = text.getText();
		if (!checkRes.equals(res[0])) {
			int dbg = 0; dbg++; //SHOULD NOT OCCUR
		}
		return res[0];
	}
